...
public class StuProcess {
	...
	Map<String, Object> stuInput(List<Student> list, int stuCount) {
		//반복
		while(true) {
			System.out.println("[ 학생정적 입력 ]");
			System.out.println(stuCount+"번쨰 이름을 입력하세요.(0.이전페이지 이동)>>");
			name = scan.next();
			if(name.equals("0")) {
		...
		System.out.println();
		//list, stuCount 2개를 보내야함 -> map안에 있음
		Map<String, Object> map = new HashMap<>();
		map.put("list", list); //list=list
		map.put("stuCount", stuCount); //stuCount=stuCount
